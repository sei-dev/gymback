...
class Validation {
	 ...
	 * @return array
	 */
	function sanitize(array $inputs, array $fields = [], int $default_filter = FILTER_SANITIZE_STRING, array $filters = self::FILTERS, bool $trim = true): array
	{
	    //die(var_dump($filters));
	    if ($fields) {
	        $options = array_map(function($field) use($filters) { return isset($filters[$field])?$filters[$field]:FILTER_SANITIZE_STRING;}, $fields);
	...
	function validate(array $data, array $fields, array $messages = []): array
	{
	    ...
	    $split = function($str, $separator) {return array_map('trim', explode($separator, $str));};
