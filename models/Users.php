...
class Users extends ModelAbstract implements ModelInterface
{
	...
	public function searchUser(string $param, int $page = 1, int $perPage = 10) {
	    
	    $start = ($page-1) * $perPage;
	    
	    $sQuery = "SELECT users.*, cities.city as location
				FROM users LEFT JOIN cities ON users.city_id = cities.id WHERE first_name LIKE '%{$param}%' OR last_name LIKE '%{$param}%' LIMIT {$start}, {$perPage};
				";
	    return $this->fetchAll($sQuery);
	}
